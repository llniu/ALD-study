...
app.layout = html.Div([
    ...
    html.Div([
        dcc.Tabs(id='tabs', value='3', children=[
            dcc.Tab(id='tab1', label='Proteome-Histology Integration', value='1', children=[
                html.Div([
                    html.Div([
                        html.P('Select a histology score here:', style=style_userinput),
                        dcc.Dropdown(id='boxplot_histology_score', options =options_histology_score, value='nas_inflam'),
                        html.P('Select a protein here: (GeneName_ProteinID)', style=style_userinput),
                        dcc.Dropdown(id='boxplot_proteinID', options =options_box_plasma_proteinID, value='PIGR__P01833'),
                        dcc.Graph(id='box_plasma'),

                    ], style={'width': '50%', 'textAlign':'center', 'padding-left':'25%'})
                ])
            ], style = tab_style, selected_style = selected_tab_style),
            dcc.Tab(id='tab2', label='Study Overview', value='2', children=[
                html.Div([
                    html.Br(),
                    html.Br(),
            ...
            dcc.Tab(id='tab3', label='Liver-Plasma Proteme Integration', value='3', children=[
                html.Div([
                    html.Div([
                        html.Br(),
                    ...
                    html.Div([
                        html.Br(),
                        html.P('Search here: (GeneName_ProteinID)', style=style_userinput),
                        dcc.Dropdown(id='pl_median_scatter_input', options=options_ploverlap, value='CRP__P02741'),
                    ], style = {'width': '60%', 'textAlign':'center', 'padding-left':'20%'}),
                    html.Div([
                        html.Br(),
...
def update_figure_plcorr_scatter(input_value):
    ...
    figure=go.Figure(data=go.Scatter(x=df['plasma'], y=df['liver'], mode='markers',
                          text=df['Genename_ProteinID'], marker_color=df['color'], marker_size=8))
...
def update_figure_plcorr_scatter(input_value):
    ...
    fig.update_layout(xaxis={'title':'Protein intensity in plasma [Log2]'},
                 yaxis={'title':'Protein intensity in liver [Log2]'},
                 width=500, plot_bgcolor='black', title='Pair-wise correlation: ' + input_value)
    return fig
...
def update_figure_plcorr_scatter(histologyscore, proteinID):
    ...
    df=df[df['Genename_ProteinID']==proteinID]
    figure=px.box(df, x=histologyscore, y="Intensity", notched=True, points='all')
    figure.update_layout(plot_bgcolor='black', yaxis={'title':'Protein intensity [Log2]'}, title='Protein levels as a function of disease severity')
