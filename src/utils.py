...
def model_performance_cv(clf, X, y, features_selected, n_repeats):
    ...
    for i, (train_index, test_index) in enumerate(rskf.split(X, y)):
        ...
        performance = model_performance(
            clf=clf, features=features_selected, X_train=X_train, X_test=X_test, y_train=y_train, y_test=y_test)
...
def feature_selection(X, y, features, n_repeats):
    performances = []
    for feature in features:
        performance = model_performance_cv(X=X, y=y, features_selected=[
                                           feature], n_repeats=n_repeats).mean()
...
def feature_selection_bestcombo(features, X_train, y_train, n_repeats=5):
    ...
    for k in range(6, 11):
        for i, j in enumerate(itertools.combinations(features, k)):
            ...
            performance = model_performance_cv(
                X=X_train, y=y_train, features_selected=combo, n_repeats=n_repeats).mean()
...
def plot_roc(clf, items, names, colors, X0, y0, X, y, title_1, title_2):
    ...
    for item in items:
        ...
        lw = 1
        plt.plot(fpr[1], tpr[1], color=colors[k], lw=lw,
                 label='{}: AUC = {}'.format(name, roc_normal))
        ...
        plt.savefig('figures/model/3C/{}_{}.png'.format(title_1,
                                                        title_2), dpi=120, bbox_inches='tight')
